...
# fnames = "ring_model_t_pdbh_STAN"
fnames = "ring_model_t_pdbh_sigd_STAN"
models = c('Model RW')
...
pfts = list(ACPE = data.frame(spcd=315, acronym="ACPE"),
...
taxaMatch$group = c(1, 1, 2, 3, 4, 5, 6)
group_name      = c('Acer', 'Acer', 'Betula', 'Fagus', 'Fraxinus', 'Picea', 'Tsuga')
taxaMatch
...
for (i in 1:N_trees) {
  ...
  # if there is a missing ring, add it back as 0 for plotting
  if (any(!(seq(min(incr_years), max(incr_years)) %in% incr_years ))){
    print(paste("Missing rings for tree", tree))
    missing_ring_idx = which(!(seq(min(incr_years), max(incr_years)) %in% incr_years ))
    N_missing_rings = length(missing_ring_idx)
    incr_tree = c(incr_tree,rep(0, N_missing_rings))
    incr_years = c(incr_years, seq(min(incr_years), max(incr_years))[missing_ring_idx])
  }
...
dbh_m_sum = aggregate(dbh~year+plot, dbh_m_melt, sum, na.rm=TRUE)
...
preds = rbind(data.frame(abi_p_melt, type="AB"),
