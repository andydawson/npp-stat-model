...
## What I have been trying to work out is what steps are necessary for the NPP-STAT...RDS file that we have been 
## using for SDA. So I'm tracing the starting data and ending data for each step. 

# TO DO:
# - make sure taxa/pft/pft_order work for buil_ab_choj function -- how species coefficients are found

#############################################################################################################
## Step 1: Estimate diameter and aboveground biomass for all trees [ab_p_1, dbh_p_1, ab_p_2, dbh_p_2]
#############################################################################################################
...
# load input data from npp stat model
fname_data = paste0('tree_data_20_', dvers)
load(file=paste0('data/', fname_data, '.rdata'))
...
# get species acronyms from input data
#spp_acro = 
...
spp_level3a = acro_level3a$level3a[match(spp_acro, acro_level3a$acronym)]

# don't know... 
# - where "taxon" is coming from
...
trees_p = sort(unique(x2tree_p)) # get unique tree names
...
pft_list_p = sort(unique(pft_list[taxon_p])) # matches all taxon to a pft 
...
build_ab_choj <- function(dbh_p, Ntrees, Nyears, keep, pfts){
  ...
  for (k in 1:keep){
    ...
    b0 = choj$beta0[match(taxon_l3a[pfts], choj$level3a)]
    b1 = choj$beta1[match(taxon_l3a[pfts], choj$level3a)]
...
choj = read.csv('data/level3a_to_chojnacky_v0.1.csv')
species2level3a = read.csv('data/acronym_to_level3a_v0.1.csv', stringsAsFactors=FALSE)
  
taxon_l3a = species2level3a$level3a[match(pft_list, species2level3a$acronym)]
  
dbh_p_1_org = abind(dbh_p_1, along=1)
dbh_p_1 = dbh_p_1_org[sort(tree_order_1, index.return=TRUE)$ix,,]
  
  ab_p_1 = array(NA, c(N_trees, N_years, keep))

for (i in 1:keep){
    
    # ln(biomass-kg) = beta0 + beta1 * ln(diameter-cm)
    b0 = choj$beta0[match(taxon_l3a[taxon], choj$level3a)]
    b1 = choj$beta1[match(taxon_l3a[taxon], choj$level3a)]
    
    
    ab_p_1[,,i] = exp(b0 + b1*log(dbh_p_1[,,i]))
  }
  ...
  ab_p_2 = array(NA, c(N_trees_p, N_years, keep))
  
  for (i in 1:keep){
    
    # ln(biomass-kg) = beta0 + beta1 * ln(diameter-cm)
    b0 = choj$beta0[match(taxon_l3a[taxon_p], choj$level3a)]
    b1 = choj$beta1[match(taxon_l3a[taxon_p], choj$level3a)]
    
    
    ab_p_2[,,i] = exp(b0 + b1*log(dbh_p_2[,,i]))
  }
...
if (run_predict){
  ...
  ab_p_1 = build_ab_choj(dbh_p_1, N_trees, N_years, keep, pft_order_1)
  ...
  ab_p_2 = build_ab_choj(dbh_p_2, N_trees, N_years, keep, pft_order_2)
...
#############################################################################################################
## Step 2: Save all tree data to RDS files for both models
#############################################################################################################

# melting all dbh and biomass files for graphing and further manipulation 
...
# put DBH values for both models into one data frame to be saved, distinguish by model type 
...
dbh_melt_all$taxon = taxaMatch[match(taxon[dbh_melt_all$tree], taxaMatch$taxon), 'species']
saveRDS(dbh_melt_all, paste0('data/HF_DBH_tree_iter_',date,'.RDS'))

# merging biomass and diameter values into a single data frame
p1 = merge(dbh_melt, ab_melt, by=c("tree", "year", "iter"))
p2 = merge(dbh2_melt, ab2_melt, by=c("tree", "year", "iter"))
p_melt_all = rbind(data.frame(p1, model=rep("Model RW + Census", nrow(p1))),
                   data.frame(p2, model=rep("Model RW", nrow(p2))))
p_melt_all$plot = tree_site_id[p_melt_all$tree]
p_melt_all$taxon = taxaMatch[match(taxon[p_melt_all$tree], taxaMatch$taxon), 'species']
p_melt_all$census_id = dbh[match(p_melt_all$tree, dbh$stat_id),'census_id']
p_melt_all$dist_census = census[match(p_melt_all$census_id, census$census_id),'dist_census']/3.28084
saveRDS(p_melt_all, paste0('data/HF_PREDS_tree_iter_',date,'.RDS'))

#############################################################################################################
## Step 3: Save all trees (< 5cm) to RDS files for both models [reduced => ab_p_1, dbh_p_1, ab_p_2, dbh_p_2]
#############################################################################################################

# remove trees that have a mean diameter less than 5 centimeters across all iterations for both models
...
# melt down, combine, and save to RDS files
dimnames(dbh_p_1)[[1]] <- seq(1, 603)
dbh_melt = melt(dbh_p_1)
colnames(dbh_melt) = c("tree", "year", "iter", "dbh")
dbh_melt$year = years[dbh_melt$year]
...
saveRDS(p_melt_all, paste0('data/HF_PREDS_tree_iter_cut_',date,'.RDS'))

#############################################################################################################
## Step 4: Adjust for mortality using census data [smoothed => ab_p_1, dbh_p_1, ab_p_2, dbh_p_2]
#############################################################################################################
...
# need data to understand this function... 
...
#############################################################################################################
## Step 5: Get dbh and ab from increments and most recent dbh measurement [dbh_m, ab_m] - need data for this
#############################################################################################################
...
dbh_m_org = abind(dbh_m_list, along=1)
dbh_m = dbh_m_org[sort(idx_m, index.return=TRUE)$ix,]

# use Chojnacky allometric coefficients to calculate biomass 
ab_m = array(NA, c(N_trees, N_years))
...
#############################################################################################################
## Step 5: Calculate biomass and biomass increment [ab_p_melt, abi_p_melt] in Mg/ha
#############################################################################################################
...
ab_p_melt$year_id    = years[ab_p_melt$year_id]

# save
saveRDS(ab_p_melt, file=paste0('output/',site,'/ab_p_', location, '_', mvers, '.RDS'))

# determine biomass increment ABCXYZ
...
saveRDS(abi_p1, paste0("output/",site,"/abi_p1_",mvers, ".rds" ))
...
######################################################################################################################################
## Unsure what steps we actually need to include. These are the steps I've identified: 
# - smooth out death using census
# 
######################################################################################################################################
...
choj = read.csv('data/level3a_to_chojnacky_v0.1.csv')
# species2level3a = read.csv('data/species_to_level3a.csv')
# species2level3a = read.csv('data/fia_to_level3a_v0.5.csv')
species2level3a = read.csv('data/acronym_to_level3a_v0.1.csv', stringsAsFactors=FALSE)

taxon_l3a = species2level3a$level3a[match(pft_list, species2level3a$acronym)]

dbh_c_org = abind(dbh_c_list, along=1)
dbh_c = dbh_c_org[sort(idx_c, index.return=TRUE)$ix,]

# ab_c = array(NA, c(N_trees, N_years))
ab_c = array(NA, c(N_trees, N_census_years))
...
colnames(ab_p_melt)[which(colnames(ab_p_melt)=="year_id")] = "year"
...
ab_c_melt$year = ab_c_melt$year_id
