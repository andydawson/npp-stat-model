...
if (!file.exists("allom/HF")){
  dir.create("allom/HF")
}

allom.fit   = load.allom('allom/HF')
if ((!(all(names(pfts) %in% names(allom.fit))))|sample_allom){
  # allom.stats = AllomAve(pfts, ngibbs=200, components=6, outdir='allom')
  allom.stats = AllomAve(pfts, ngibbs=200, components=component, outdir='allom/HF')
  allom.fit   = load.allom('allom/HF')
}
...
saveRDS(dbh_melt_all, 'data/HF_DBH_tree_iter_29May2019.RDS') 
...
saveRDS(p_melt_all, 'data/HF_PREDS_tree_iter_29May2019.RDS')
...
saveRDS(p_melt_all, 'data/HF_PREDS_tree_iter_cut_29May2019.RDS')
######################################################################################################################################
...
# idx_out_outer = unique(dbh_melt_all[which((dbh_melt_all$year==2015)&(dbh_melt_all$dist_census>13)&(dbh_melt_all$dbh<20)), 'tree'])
# idx_out_inner = unique(dbh_melt_all[which((dbh_melt_all$year==2015)&(dbh_melt_all$dbh<10)),'tree'])
# 
# dbh_melt_all = dbh_melt_all[which(!(dbh_melt_all$tree %in% idx_out_outer)),]
# dbh_melt_all = dbh_melt_all[which(!(dbh_melt_all$tree %in% idx_out_inner)),]
# dbh_melt_all = dbh_melt_all[which(dbh_melt_all$dbh>=12.7),]
